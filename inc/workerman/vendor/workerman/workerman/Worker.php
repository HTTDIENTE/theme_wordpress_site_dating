...
class Worker {
	...
	protected static function displayUI() {
		...
		//show version
		$line_version = 'Workerman version:' . static::VERSION . \str_pad( 'PHP version:', 22, ' ',
				\STR_PAD_LEFT ) . \PHP_VERSION;
		...
		$line_one     = '<n>' . \str_pad( '<w> WORKERMAN </w>', $total_length + \strlen( '<w></w>' ), '-',
				\STR_PAD_BOTH ) . '</n>' . \PHP_EOL;
		$line_two     = \str_pad( '<w> WORKERS </w>', $total_length + \strlen( '<w></w>' ), '-',
		...
		foreach ( static::getUiColumns() as $column_name => $prop ) {
			...
			$title .= "<w>{$column_name}</w>" . \str_pad( '',
					static::$$key + static::UI_SAFE_LENGTH - \strlen( $column_name ) );
		}
		...
		foreach ( static::$_workers as $worker ) {
			...
			foreach ( static::getUiColumns() as $column_name => $prop ) {
				...
				$content             .= \str_pad( (string) $worker->{$prop},
					static::$$key + static::UI_SAFE_LENGTH + $place_holder_length );
			}
	...
	protected static function formatStatusData( $statistics_file ) {
		...
		foreach ( $info as $key => $value ) {
			...
			if ( \preg_match( '/^[0-9]+/', $value, $pid_math ) ) {
				...
				if ( \preg_match( '/^\S+?\s+?(\S+?)\s+?(\S+?)\s+?(\S+?)\s+?(\S+?)\s+?(\S+?)\s+?(\S+?)\s+?(\S+?)\s+?/',
					$value, $match ) ) {
	...
	protected static function forkWorkersForWindows() {
		...
		if ( \in_array( '-q', $argv ) || \count( $files ) === 1 ) {
			...
			// Display UI.
			static::safeEcho( \str_pad( $worker->name, 30 ) . \str_pad( $worker->getSocketName(),
					36 ) . \str_pad( $worker->count, 10 ) . "[ok]\n" );
			$worker->listen();
			$worker->run();
			exit( "@@@child exit@@@\r\n" );
		} else {
	...
	public function setUserAndGroup() {
		...
		if ( $uid !== \posix_getuid() || $gid !== \posix_getgid() ) {
			if ( ! \posix_setgid( $gid ) || ! \posix_initgroups( $user_info['name'],
					$gid ) || ! \posix_setuid( $uid ) ) {
	...
	protected static function writeStatisticsToStatusFile() {
		...
		if ( static::$_masterPid === \posix_getpid() ) {
			...
			$loadavg = \function_exists( 'sys_getloadavg' ) ? \array_map( 'round', \sys_getloadavg(),
				array( 2, 2, 2 ) ) : array( '-', '-', '-' );
			...
			foreach ( static::$_pidMap as $worker_id => $worker_pid_array ) {
				...
				if ( isset( static::$_globalStatistics['worker_exit_info'][ $worker_id ] ) ) {
					foreach ( static::$_globalStatistics['worker_exit_info'][ $worker_id ] as $worker_exit_status => $worker_exit_count ) {
						\file_put_contents( static::$_statisticsFile,
							\str_pad( $worker->name,
								static::$_maxWorkerNameLength ) . " " . \str_pad( $worker_exit_status,
			...
			\file_put_contents( static::$_statisticsFile,
				"pid\tmemory  " . \str_pad( 'listening',
					static::$_maxSocketNameLength ) . " " . \str_pad( 'worker_name',
		...
		/** @var \Workerman\Worker $worker */
		$worker            = current( static::$_workers );
		$worker_status_str = \posix_getpid() . "\t" . \str_pad( round( memory_get_usage( false ) / ( 1024 * 1024 ),
					2 ) . "M", 7 )
		                     . " " . \str_pad( $worker->getSocketName(), static::$_maxSocketNameLength ) . " "
		                     . \str_pad( ( $worker->name === $worker->getSocketName() ? 'none' : $worker->name ),
				static::$_maxWorkerNameLength )
		                     . " ";
	...
	protected static function writeConnectionsStatisticsToStatusFile() {
		...
		foreach ( TcpConnection::$connections as $connection ) {
			...
			$str .= \str_pad( $pid, 9 ) . \str_pad( $worker_name, 16 ) . \str_pad( $id, 10 ) . \str_pad( $transport, 8 )
			        ...
			        . \str_pad( $state, 14 ) . ' ' . \str_pad( $local_address, 22 ) . ' ' . \str_pad( $remote_address,
					22 ) . "\n";
		}
		if ( $str ) {
	...
	public static function log( $msg ) {
		...
		\file_put_contents( (string) static::$logFile, \date( 'Y-m-d H:i:s' ) . ' ' . 'pid:'
		                                               . ( static::$_OS === \OS_TYPE_LINUX ? \posix_getpid() : 1 ) . ' ' . $msg,
			\FILE_APPEND | \LOCK_EX );
	}
