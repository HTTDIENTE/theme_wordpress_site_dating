...
class Request {
	...
	public function sessionId( $session_id = null ) {
		...
		if ( ! isset( $this->sid ) ) {
			...
			if ( $sid === '' || $sid === null ) {
				...
				$this->connection->__header['Set-Cookie'] = array(
					$session_name . '=' . $sid
	...
	protected function parseUploadFiles( $http_post_boundary ) {
		...
		while ( $max_count -- > 0 && $offset ) {
			$offset = $this->parseUploadFile( $http_post_boundary, $offset, $post_encode_string, $files_encode_string,
				$files );
		}
	 ...
	 * @return int
	 */
	protected function parseUploadFile(
		$boundary,
		...
		$content_lines_str = \substr( $this->_buffer, $section_start_offset,
			$content_lines_end_offset - $section_start_offset );
		$content_lines     = \explode( "\r\n", trim( $content_lines_str . "\r\n" ) );
		$boundary_value    = \substr( $this->_buffer, $content_lines_end_offset + 4,
			$section_end_offset - $content_lines_end_offset - 4 );
		$upload_key        = false;
