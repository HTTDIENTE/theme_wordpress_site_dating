	...
	public static function dealHandshake( $buffer, TcpConnection $connection ) {
		...
		if ( $pos ) {
			...
			if ( \preg_match( "/Sec-WebSocket-Accept: *(.*?)\r\n/i", $buffer, $match ) ) {
				if ( $match[1] !== \base64_encode( \sha1( $connection->websocketSecKey . "258EAFA5-E914-47DA-95CA-C5AB0DC85B11",
						true ) ) ) {
			...
			if ( isset( $connection->onWebSocketConnect ) ) {
				try {
					\call_user_func( $connection->onWebSocketConnect, $connection,
						\substr( $buffer, 0, $handshake_response_length ) );
				} catch ( \Exception $e ) {
					Worker::stopAll( 250, $e );
				} catch ( \Error $e ) {
					Worker::stopAll( 250, $e );
			...
			if ( ! empty( $connection->websocketPingInterval ) ) {
				$connection->websocketPingTimer = Timer::add( $connection->websocketPingInterval,
