...
class Timer {
			...
			if ( $time_now >= $run_time ) {
				foreach ( $task_data as $index => $one_task ) {
					...
					if ( $persistent && ! empty( self::$_status[ $index ] ) ) {
						...
						if ( ! isset( self::$_tasks[ $new_run_time ] ) ) {
							self::$_tasks[ $new_run_time ] = array();
						}
						self::$_tasks[ $new_run_time ][ $index ] = array(
							$task_func,
							(array) $task_args,
							$persistent,
							$time_interval
	...
	public static function del( $timer_id ) {
		...
		foreach ( self::$_tasks as $run_time => $task_data ) {
			if ( array_key_exists( $timer_id, $task_data ) ) {
				unset( self::$_tasks[ $run_time ][ $timer_id ] );
			}
		}

		if ( array_key_exists( $timer_id, self::$_status ) ) {
			unset( self::$_status[ $timer_id ] );
		}
